...
const CampaignDetailPage = () => {
  ...
  const [newVideo, setNewVideo] = useState({
    title: '', url: '', cpc: 0, views: 0, engagement: 0, likes: 0, shares: 0, comments: 0
  });
  const [newHypothesis, setNewHypothesis] = useState({
    type: 'Problema', condition: '', validation_status: 'No Validada'
  });
  ...
  const handleCreateVideo = async (e) => {
    ...
    if (result) {
      setNewVideo({ title: '', url: '', cpc: 0, views: 0, engagement: 0, likes: 0, shares: 0, comments: 0 });
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Button
                  onClick={() => setShowAudienceForm(!showAudienceForm)}
              ...
              {showAudienceForm && (
                <form onSubmit={handleCreateAudience} className="mb-4 space-y-3 p-4 bg-blue-50 rounded-lg">
                  <input
                    type="text"
                  ...
                  <div className="flex gap-2">
                    <Button type="submit" className="flex-1 bg-blue-500 hover:bg-blue-600 text-white text-sm">
                      Create
                    </Button>
                    <Button
                      type="button"
                      onClick={() => setShowAudienceForm(false)}
                      className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm"
                    >
                      Cancel
                    </Button>
                  </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              ...
              {!selectedAudience && (
                <div className="text-center py-8 text-gray-500 text-sm">
                  Select an audience to view videos
                </div>
              )}
                ...
                <form onSubmit={handleCreateVideo} className="mb-4 space-y-2 p-4 bg-purple-50 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      step="0.01"
                      value={newVideo.cpc}
                      onChange={(e) => setNewVideo({ ...newVideo, cpc: e.target.value })}
                      className="px-3 py-2 border-2 border-gray-200 rounded-lg text-gray-900 placeholder-gray-400 text-sm"
                      placeholder="CPC"
                    />
                    <input
                      type="number"
                    ...
                    <input
                      type="number"
                      step="0.01"
                      value={newVideo.engagement}
                      onChange={(e) => setNewVideo({ ...newVideo, engagement: e.target.value })}
                      className="px-3 py-2 border-2 border-gray-200 rounded-lg text-gray-900 placeholder-gray-400 text-sm"
                    ...
                    <input
                      ...
                      onChange={(e) => setNewVideo({ ...newVideo, likes: e.target.value })}
                      className="px-3 py-2 border-2 border-gray-200 rounded-lg text-gray-900 placeholder-gray-400 text-sm"
                      placeholder="Likes"
                    />
                    <input
                      type="number"
                      value={newVideo.shares}
                      onChange={(e) => setNewVideo({ ...newVideo, shares: e.target.value })}
                      className="px-3 py-2 border-2 border-gray-200 rounded-lg text-gray-900 placeholder-gray-400 text-sm"
                      placeholder="Shares"
                    />
                    <input
                      ...
                      onChange={(e) => setNewVideo({ ...newVideo, comments: e.target.value })}
                      className="px-3 py-2 border-2 border-gray-200 rounded-lg text-gray-900 placeholder-gray-400 text-sm"
                      placeholder="Comments"
                    />
                  </div>
                  <div className="flex gap-2">
                    <Button type="submit" className="flex-1 bg-purple-500 hover:bg-purple-600 text-white text-sm">
                      Create
                    </Button>
                    <Button
                      type="button"
                  ...
                  {videos.map((video) => (
                    <div key={video.id} className="p-3 rounded-lg border-2 border-gray-200 hover:border-purple-300 transition-all">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <Button
                          onClick={() => deleteVideo(video.id, selectedAudience)}
                          className="bg-red-100 hover:bg-red-200 text-red-600 p-1 h-auto"
                        >
                          <Trash2 className="w-3 h-3" />
                        </Button>
              ...
              {showHypothesisForm && (
                <form onSubmit={handleCreateHypothesis} className="mb-4 space-y-3 p-4 bg-amber-50 rounded-lg">
                  <select
                    ...
                    onChange={(e) => setNewHypothesis({ ...newHypothesis, type: e.target.value })}
                  ...
                  <textarea
                    ...
                    onChange={(e) => setNewHypothesis({ ...newHypothesis, condition: e.target.value })}
                    className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-gray-900 placeholder-gray-400 text-sm"
                    placeholder="Condition (e.g., views > 1000)"
                    rows="2"
                    required
                  />
                  <div className="flex gap-2">
                    <Button type="submit" className="flex-1 bg-amber-500 hover:bg-amber-600 text-white text-sm">
                      Create
                    </Button>
                    <Button
                      type="button"
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Status: <span className={hypothesis.validation_status === 'Validada' ? 'text-green-600 font-semibold' : 'text-gray-600'}>
                            {hypothesis.validation_status}
                          </span>
                        </p>
                      </div>
                      <Button
                        onClick={() => deleteHypothesis(hypothesis.id, id)}
                        className="bg-red-100 hover:bg-red-200 text-red-600 p-1 h-auto"
                      >
                        <Trash2 className="w-3 h-3" />
                      </Button>
